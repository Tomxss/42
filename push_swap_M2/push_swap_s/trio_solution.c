...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/20 12:26:07 by tcoetzee          #+#    #+#             */
/*   Updated: 2019/09/20 12:28:40 by tcoetzee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_oper	*steps_for_trio(t_stack *stacks, t_oper *steps)
{
	...
	else if (stacks->a_mxi == stacks->a_topi && stacks->a_mni == stacks->\
			a_topi + 2)
		steps = add_step(add_step(steps, 5), 0);
	else if (stacks->a_mxi == stacks->a_topi && stacks->a_mni == stacks->\
			a_topi + 1)
		steps = add_step(steps, 5);
	else if (stacks->a_mxi == stacks->a_topi + 2 && stacks->a_mni == stacks->\
			a_topi + 1)
		steps = add_step(steps, 0);
	else if (stacks->a_mxi == stacks->a_topi + 1 && stacks->a_mni == stacks->\
			a_topi + 2)
