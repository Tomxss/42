...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/16 16:59:56 by tcoetzee          #+#    #+#             */
/*   Updated: 2019/09/16 17:03:06 by tcoetzee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_oper	*new_step(int oper)
{
	...
	if (!new)
		return (NULL);
	new->operations = get_step_name(oper);
	if (!new->operations)
	{
		free(new);
		return (NULL);
...
t_oper			*steps_for_trio(t_stack *stacks, t_oper *steps)
{
	...
	else if (stacks->a_mxi == stacks->a_top && \
			stacks->a_mni == stacks->a_top + 2)
		steps = add_step(add_step(steps, 5), 0);
	else if (stacks->a_mxi == stacks->a_top && \
			stacks->a_mni == stacks->a_top + 1)
		steps = add_step(steps, 5);
	else if (stacks->a_mxi == stacks->a_top + 2 \
			&& stacks->a_mni == stacks->a_top + 1)
		steps = add_step(steps, 0);
	else if (stacks->a_mxi == stacks->a_top + 1 \
			&& stacks->a_mni == stacks->a_top + 2)
