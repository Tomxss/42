...
static int stack_is_empty(t_stack *stacks, char c)
{
  ...
  if (c == 'a')
  {
    if ((stacks->a_amt_of_elems < stacks->size) && stacks->a_print_flag == 0)
  ...
  if (c == 'b')
  {
    if ((stacks->b_amt_of_elems < stacks->size) && stacks->b_print_flag == 0)
